...
class InMemoryApprovalStore : ApprovalStore {
    ...
    @Synchronized override fun save(context: ApprovalContext) {
        approvals[context.approvalId] = context
    }

    @Synchronized override fun load(approvalId: String): ApprovalContext? = approvals[approvalId]

    @Synchronized override fun consume(
        approvalId: String,
        runId: String,
        requesterIdentity: String,
        fingerprint: String,
        ...
        if (current.runId != runId || current.requesterIdentity != requesterIdentity || current.fingerprint != fingerprint) {
            return ApprovalConsumption.CONFLICT
        }
...
class JournalApprovalStore(private val file: File) : ApprovalStore {
    ...
    override fun load(approvalId: String): ApprovalContext? = synchronized(lock) {
        replay()[approvalId]?.toContext()
    }
    ...
    override fun consume(
        ...
        decision: ApprovalDecision,
        approverIdentity: String
    ): ApprovalConsumption = synchronized(lock) {
        ...
        if (current.runId != runId || current.requesterIdentity != requesterIdentity || current.fingerprint != fingerprint) {
            return@synchronized ApprovalConsumption.CONFLICT
        }
        append(Record.encode(current.copy(status = decision.toStatus(), resolvedBy = approverIdentity)))
    ...
    private fun replay(): Map<String, Record> {
        ...
        file.forEachLine(Charsets.UTF_8) { line ->
            ...
            Record.decode(line.split('|'))?.let { records[it.approvalId] = it }
        }
        return records
    }
    ...
    private data class Record(
        ...
        companion object {
            fun encode(context: ApprovalContext): String = listOf(
                "A", Codec.encode(context.approvalId), Codec.encode(context.runId), Codec.encode(context.requesterIdentity),
                Codec.encode(context.actionId), context.actionVersion.toString(), Codec.encodeMap(context.input),
                Codec.encodeInvocations(context.plannedInvocations), Codec.encode(context.fingerprint), context.status.name,
                Codec.encodeNullable(context.resolvedBy)
            ).joinToString("|")

            fun decode(parts: List<String>): Record? {
                if (parts.size != 11 || parts[0] != "A") return null
                return runCatching {
                    Record(
                        approvalId = Codec.decode(parts[1]), runId = Codec.decode(parts[2]), requesterIdentity = Codec.decode(parts[3]),
                        actionId = Codec.decode(parts[4]), actionVersion = parts[5].toInt(), input = Codec.decodeMap(parts[6]),
                        plannedInvocations = Codec.decodeInvocations(parts[7]), fingerprint = Codec.decode(parts[8]),
                        status = ApprovalStatus.valueOf(parts[9]), resolvedBy = Codec.decodeNullable(parts[10])
                    )
                }.getOrNull()
            }
        }
    ...
    private object Codec {
        ...
        fun decodeMap(value: String): Map<String, String> = if (value.isBlank()) emptyMap() else value.split(',').associate { raw ->
            val i = raw.indexOf('.')
            require(i > 0)
            decode(raw.substring(0, i)) to decode(raw.substring(i + 1))
        }
        ...
        fun encodeInvocations(value: List<CapabilityInvocationSpec>): String = value.joinToString("~") { spec ->
            listOf(encode(spec.capabilityId), encodeSet(spec.scope), encodeMap(spec.parameters), Codec.encodeNullable(spec.idempotencyKey)).joinToString("^")
        }
        fun decodeInvocations(value: String): List<CapabilityInvocationSpec> = if (value.isBlank()) emptyList() else value.split('~').map { raw ->
            val p = raw.split('^')
            require(p.size == 4)
            CapabilityInvocationSpec(decode(p[0]), decodeSet(p[1]), decodeMap(p[2]), decodeNullable(p[3]))
        }
    }
}

private fun ApprovalDecision.toStatus(): ApprovalStatus = when (this) {
