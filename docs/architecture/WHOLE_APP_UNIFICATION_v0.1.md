...
The existing Anthropic conversation adapter remains responsible for provider transport and streaming. Local tool execution is being moved behind the runtime's Action/Capability boundary. MCP transport remains provider-owned until a native internal MCP adapter exists.
...
- A provider adapter cannot bypass local policy or approval by issuing a provider-side tool call.
...
7. Introduce explicit EgressDecision before protected remote data flows.
8. Extract MCP into an internal protocol adapter whose effects also converge on runtime semantics where locally authorized.
9. Add Android lifecycle/process-death integration tests.
10. Only then expand activation surfaces.

## Promotion criterion

The migration is not considered complete while an effect can execute through a path other than `CapabilityExecutor`, or while the UI/model path and runtime path maintain conflicting authority semantics.
